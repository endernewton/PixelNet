...
// compute the nnz
template <typename Dtype>
void caffe_gpu_nnz(const int M, const int N, const Dtype* A, 
  int* nnzPerRowColumn, int* nnzTotalDevHostPtr);

// dense matrix to csr format
template <typename Dtype>
void caffe_gpu_dense2csr(const int M, const int N, const Dtype* A, const int* nnzPerRow, 
  Dtype* csrValA, int* csrRowPtrA, int* csrColIndA);

// matrix multiplication, just for my purpose
template <typename Dtype>
void caffe_gpu_csrmm(const int M, const int N, const int K, const int nnz, 
    const Dtype alpha, const Dtype* csrValA, const int* csrRowPtrA, const int* csrColIndA, 
    const Dtype* B, const int ldb, const Dtype beta, Dtype* C);
